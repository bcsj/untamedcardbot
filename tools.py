...
def print_card_short(card):
    msg = factionemoji(card) + ' ' + card['name'] + ', ' + card['type'] + (' :star:' if card['rally'] else '')
    return msg
...
def print_card(card):
    ...
    msg = card['name'] + ', ' + factionemoji(card) + ' ' + card['faction'] + ' faction' + '\n' \
            + card['type'] + (' :star:' if card['rally'] else '') + (', ' + cost_emoji + ' ' + str(card['cost']) if card['cost'] is not None else '')  \
